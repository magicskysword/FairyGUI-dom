...
export class Transition {
    ...
    public playReverse(onComplete?: Function, times?: number, delay?: number, startTime?: number, endTime?: number): void {
    ...
    private _play(onComplete?: Function, times?: number, delay?: number, startTime?: number, endTime?: number, reversed?: boolean): void {
        this.stop(true, true);
    ...
    public stop(setToComplete?: boolean, processCallback?: boolean): void {
        ...
        this._totalTasks = 0;
        this._totalTimes = 0;
        var handler: Function = this._onComplete;
    ...
    public setHook(label: string, callback: Function): void {
        var cnt: number = this._items.length;
        var found: boolean = false;
        for (var i: number = 0; i < cnt; i++) {
            var item: Item = this._items[i];
