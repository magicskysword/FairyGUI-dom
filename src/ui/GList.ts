...
export class GList extends GComponent {
    ...
    private _triggerFocusEvents?: boolean;
    private _keySelectEvent?: string;
    ...
    public get virtualItemSize(): Vec2 {
        return this._itemSize;
    }

    public set virtualItemSize(value: Vec2) {
        if (this._virtual) {
    ...
    public setDefaultItemSize(sizeX: number, sizeY: number) {
    ...
    public enableSelectionFocusEvents(enabled: boolean) {
    ...
    private notifySelection(evt: Event) {
    ...
    public handleArrowKey(dir: number): number {
        var curIndex: number = this.selectedIndex;
        if (curIndex == -1) {
            ...
            case 1://up
                if (this._layout == ListLayoutType.SingleColumn || this._layout == ListLayoutType.FlowVertical) {
                    ...
                    if (this._virtual) {
                        ...
                        let current: GObject = this._children[index];
                        let k = 0;
                        let i;
                        for (i = index - 1; i >= 0; i--) {
                            var obj: GObject = this._children[i];
    ...
    public getSnappingPositionWithDir(xValue: number, yValue: number, xDir: number, yDir: number, resultPoint?: Vec2): Vec2 {
        if (this._virtual) {
            if (!resultPoint)
    ...
    public get touchItem(): GObject | null {
        ...
        if (this._virtual) {
            ...
            if (this._realNumItems > oldCount) {
                for (i = oldCount; i < this._realNumItems; i++) {
                    var ii: ItemInfo = { width: 0, height: 0, flag: 0 };
